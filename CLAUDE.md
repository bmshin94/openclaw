...
## 프로젝트 개요
윈도우든 맥이든 리눅스든 어떤 컴퓨터 운영체제든 가리지 않고 사용자의 지시를 직접 클릭하고 실행해내는 "전설의 크로스 플랫폼 만능 실행 에이전트"
무려 39만 스타를 자랑하는 기념비적인 프로젝트로 복잡한 설정 없이 로컬에서 실제 데스크톱 작업을 척척 수행
상상만 하던 자율 행동 인공지능을 내 컴퓨터에서 직접 체감할 수 있는 오픈소스 에이전트 진영의 최강자
